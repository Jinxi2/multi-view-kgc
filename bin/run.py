...
""" Train and test the CoKE model on knowledge graph completion
"""
...
def kbc_predict(test_exe, test_program, test_pyreader, fetch_list, all_examples, true_triplets_dict, eval_result_file):
	...
	while True:
		try:
			...
			if step % 10 == 0:
				print("Processing kbc_predict step: %d exmaples:%d" % (step, eval_i))
			step += 1
			eval_i += _batch_len
...
def predict(test_exe, test_program, test_pyreader, fetch_list, all_examples, args):
	...
	print(">> Evaluation result file: %s" % eval_result_file)
...
def _load_kbc_eval_dict(true_triple_file):
	...
	true_triples_dict = collections.defaultdict(lambda: {'hs': collections.defaultdict(list), 'ts': collections.defaultdict(list)})
...
def init_coke_net_config(args, print_config = True):
	...
	if print_config is True:
		print('----------- CoKE Network Configuration -------------')
...
def main(args):
			...
			try:
				...
				if steps % args.skip_steps == 0:
					...
					if args.verbose:
						...
						verbose += "learning rate: %f" % (np_lr[0] if warmup_steps > 0 else args.learning_rate)
						print(verbose)
					...
					time_begin = time.time()
					if epoch % 200 == 0:
						save_path = os.path.join(args.checkpoints, "epoch_" + str(epoch))
				...
				break
		print(">>Finish training at %s " % time.ctime())
	...
	if args.do_predict:
		...
		print(">>Finish predicting at %s " % time.ctime())
